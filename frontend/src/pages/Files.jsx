...
export default function FilesPage() {
  ...
  const handleDownload = (f) => {
    ...
    window.open(filesApi.downloadUrl(selF, f.message_id), "_blank");
  ...
  // current file combined progress
        ...
        <div style={{flex:1,overflow:"auto",padding:22}}>
          ...
          {hasQueue && (
              <div style={{marginBottom:16,background:"var(--bg2)",border:"1px solid var(--border)",borderRadius:12,overflow:"hidden"}}>
                {/* summary bar — always visible, clickable to expand */}
                <div className="flex items-center justify-between"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {isFinished && (
                        <button className="btn btn-ghost btn-sm" onClick={(e)=>{e.stopPropagation();resetUpload();}} style={{padding:"4px 10px",fontSize:11}}>Limpar</button>
                    )}
                    ...
                    <div style={{borderTop:"1px solid var(--border)",padding:"8px 10px",maxHeight:300,overflow:"auto"}}>
                      {uploadQueue.map(item => (
                          <div key={item.id} className="flex items-center gap-2" style={{
                            ...
                            {/* controls — only for pending/paused items (NOT the current uploading one) */}
                            {item.id !== uploadCurrentId && (item.status==="pending"||item.status==="paused") && (
                                <div className="flex gap-2" style={{flexShrink:0}} onClick={e=>e.stopPropagation()}>
                                  {item.status==="paused" ? (
                                      <button className="btn btn-ghost btn-sm" onClick={()=>resumeQueueItem(item.id)} style={{padding:"2px 8px",fontSize:10}}>
                                        <Icon n="play" size={10}/>
                                      </button>
                                  ) : (
                                      <button className="btn btn-ghost btn-sm" onClick={()=>pauseQueueItem(item.id)} style={{padding:"2px 8px",fontSize:10}}>
                                        <Icon n="pause" size={10}/>
                                      </button>
                                  )}
                                  <button className="btn btn-ghost btn-sm" onClick={()=>cancelQueueItem(item.id)} style={{padding:"2px 8px",fontSize:10,color:"var(--red)"}}>
