...
import { Icon, Toggle, splitName } from "../components/common.jsx";
...
export default function SettingsPage() {
  ...
  useEffect(()=>{
    fetch((import.meta.env.VITE_API_URL||"http://localhost:8001")+"/api/health")
      ...
      <div style={{maxWidth:540,display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div className="card anim-fu2" style={{padding:20}}>
          <div className="flex items-center justify-between" style={{marginBottom:14}}>
            <div style={{fontSize:10,fontWeight:600,color:"var(--text3)",textTransform:"uppercase",letterSpacing:".1em"}}>Preferências</div>
            <span className="tag tag-yellow" style={{fontSize:9}}>EM BREVE</span>
          </div>
          {[
            {label:"Notificações",          sub:"Alertas ao concluir ou falhar backups"},
            {label:"Iniciar com o sistema", sub:"Inicia automaticamente no login do KDE"},
          ...
          <div style={{fontSize:11,color:"var(--text3)",marginTop:8,padding:"8px 10px",background:"var(--bg)",borderRadius:8,border:"1px solid var(--border)"}}>
            Essas preferências serão funcionais quando o TeleVault for empacotado como app desktop (systemd + tray).
          </div>
        </div>
